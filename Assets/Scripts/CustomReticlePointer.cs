...
public class CustomReticlePointer : GvrBasePointer
{
    /// <summary>
    ...
    public const float RETICLE_MIN_OUTER_ANGLE = 0.4f;
    ...
    public float reticleGrowthSpeed = 4.0f;
    ...
    /// Custom Additions
    /// </summary>
    private float duration = 0.5f; //reticle duration in seconds
    private float timer; //track reticle progress
    private bool filling = false; //reticle filled property


    private GameObject target;

    public PointerEventData eventdata = new PointerEventData(EventSystem.current);

    public void StartReticleProgress()
    {
        filling = true;
        timer = 0f;
    }

    public void ResetReticleProgress()
    {
        filling = false;
        target = null;
        timer = 0f;
    ...
    public bool IncrementReticleProgress()
    {
    ...
    public override void OnPointerHover(RaycastResult raycastResultResult, bool isInteractive)
    {
        ...
        if (IncrementReticleProgress())
        {
            target = raycastResultResult.gameObject;
            if (target.GetComponent<EventTrigger>() != null)
            {
                target.GetComponent<EventTrigger>().OnPointerClick(eventdata);
            }
        }
    }
    ...
    public void UpdateDiameters()
    {
      ...
      Mathf.Clamp(ReticleDistanceInMeters, RETICLE_DISTANCE_MIN, maxReticleDistance);
        ...
        {
            ReticleInnerDiameter = RETICLE_MIN_INNER_ANGLE;
          //Mathf.Lerp(ReticleInnerDiameter, inner_diameter, Time.unscaledDeltaTime * reticleGrowthSpeed);
            ReticleOuterDiameter =
          Mathf.Lerp(ReticleOuterDiameter, outer_diameter, Time.unscaledDeltaTime * reticleGrowthSpeed);
        }
